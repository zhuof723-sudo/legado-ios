...
public struct SearchResult: Codable, Sendable {
    ...
    public init(
        name: String, author: String, intro: String, kind: String,
        lastChapter: String, bookUrl: String, coverUrl: String, wordCount: String
    ) {
        self.name = name; self.author = author; self.intro = intro; self.kind = kind
        self.lastChapter = lastChapter; self.bookUrl = bookUrl
...
public final class BookSourceRuntime {
    ...
    /// 原始评论数据（键：userName/avatarUrl/content/postTime/likeCount/replyCount）
    public typealias RawReview = [String: String]

    /// 获取某段的评论列表。
    /// reviewUrl 中支持占位符：{{chapterUrl}}、{{paragraphText}}、{{page}}
    public func getReviews(
        ...
        // paragraphText 需要 URL 编码
        if let encodedText = paragraphText.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed) {
            url = url.replacingOccurrences(of: "{{paragraphText}}", with: encodedText)
        }

        EngineLogger.log("段评请求: \(url)", tag: source.bookSourceName)
        await SourceRateLimiter.shared.acquire(key: source.bookSourceUrl, concurrentRate: source.concurrentRate)
        defer { SourceRateLimiter.shared.release(key: source.bookSourceUrl) }
        ...
        // 解析评论列表
        // 书源可以用 ruleReview.contentRule 指定评论列表的选择器（返回 HTML 片段列表）
        ...
        if let contentRule = rule?.contentRule, !contentRule.isEmpty {
            ...
            // 没有配置 contentRule，尝试直接解析整个响应为评论内容
            let text = analyzeRule.getString("@content")
            if !text.isEmpty {
                var review: RawReview = [:]
                review["content"] = text
                review["userName"] = "匿名"
        ...
        EngineLogger.log("段评获取到 \(reviews.count) 条", tag: source.bookSourceName)
        return reviews
    }
