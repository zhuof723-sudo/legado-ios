    ...
    private func loadCurrentContent() async {
        ...
        do {
            ...
            if let bookURL = persistentBookURL {
                ...
                text = try await runtime.getContent(chapterUrl: chapters[currentIndex].url)
            }
            ...
            Task { [weak self] in
                ...
                do {
                    ...
                    if let bookURL = self.persistentBookURL {
                        ...
                        text = try await self.runtime.getContent(chapterUrl: self.chapters[i].url)
                    }
                    guard !text.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else {
                        await MainActor.run { self.prefetchTasks.remove(i) }
                        return
                    }
                    await MainActor.run {
                    ...
                    if let bookURL = self.persistentBookURL {
                        await ChapterContentCache.shared.save(
                            text, bookURL: bookURL, chapterURL: self.chapters[i].url
                        )
                    }
                } catch {
                    // 预取失败静默处理，不影响当前阅读
